...
// --- Movement Settings ---
const float STAND_HEIGHT =
    15.0;                      // Height of foot from hip in stand position (cm)
const float STEP_LENGTH = 4.0; // How far forward/back the leg swings (cm)
const float STEP_HEIGHT = 6.0; // How high the leg lifts during swing (cm)
...
const bool INVERT_KNEE_ANGLE =
    true; // <-- CHANGE THIS based on your mechanical mounting

// Add to GLOBAL CONFIG section
#define CALIBRATION_MODE true // Set to true to enable leg-by-leg testing

// Add this function AFTER updateGait()
// Add to GLOBAL CONFIG
#define CALIBRATION_LEG -1     // -1 = cycle all, 0-3 = test specific leg
#define USE_LED_INDICATOR true // Blink LED on "forward" swing phase
...
// Enhanced calibration mode with direction indicators
void calibrationMode() {
  static int testLeg = 0;
  static unsigned long legTestStart = 0;
  static bool wasInForwardSwing = false;
  const int LEG_TEST_DURATION =
      4000; // 4 seconds per leg (slower = easier to observe)

  // Cycle through legs
  if (millis() - legTestStart > LEG_TEST_DURATION) {
    legTestStart = millis();

    // If testing specific leg, stay on it; otherwise cycle
    if (CALIBRATION_LEG == -1) {
      testLeg = (testLeg + 1) % NUM_LEGS;
    }

    Serial.println("\n========================================");
    Serial.print(">>> NOW TESTING LEG: ");
    printLegName(testLeg);
    Serial.println(" <<<");
    Serial.println("Expected motion for FORWARD walking:");
    printExpectedMotion(testLeg);
    Serial.println("========================================\n");
  }

  // Calculate motion for test leg
  for (int i = 0; i < NUM_LEGS; i++) {
    if (i == testLeg || CALIBRATION_LEG == i) {
      // Slow, obvious sine wave for testing
      float phase = millis() * 0.002;           // Slower motion
      float x = sin(phase) * STEP_LENGTH * 2.0; // Exaggerated swing
      float y = STAND_HEIGHT - abs(cos(phase)) * STEP_HEIGHT * 2.5; // High lift

      // LED indicator: ON when leg is in "forward swing" phase
      if (USE_LED_INDICATOR && (i == testLeg || CALIBRATION_LEG == i)) {
        bool isInForwardSwing = (sin(phase) > 0.5); // Top half of forward swing
        if (isInForwardSwing && !wasInForwardSwing) {
          digitalWrite(LED_PIN, HIGH); // Blink ON at start of forward swing
        } else if (!isInForwardSwing && wasInForwardSwing) {
          digitalWrite(LED_PIN, LOW);
        }
        wasInForwardSwing = isInForwardSwing;
      }

      allLegs[i]->setTarget(x, y);

// Print real-time direction hint (optional, verbose)
#ifdef VERBOSE_CALIBRATION
      if (millis() % 500 < 50) { // Print ~2x per second
        Serial.print("Leg ");
        printLegName(i);
        Serial.print(" | X=");
        Serial.print(x, 1);
        Serial.print(" → ");
        if (x > 2.0)
          Serial.print("SWINGING FORWARD ✓");
        else if (x < -2.0)
          Serial.print("PUSHING BACK ✓");
        else
      ...
      // Other legs stay in neutral stand
...
// Helper: Print leg name (FL, FR, RL, RR)
void printLegName(int legIndex) {
  const char *names[] = {"FL (Front-Left)", "FR (Front-Right)",
                         "RL (Rear-Left)", "RR (Rear-Right)"};
  Serial.print(names[legIndex]);
}

// Helper: Print expected motion for a given leg
void printExpectedMotion(int legIndex) {
  Serial.print("  • When X is POSITIVE (+): Leg swings ");
  switch (legIndex) {
  case FL:
    Serial.println("FORWARD-LEFT ↗ (toward robot front)");
    break;
  case FR:
    Serial.println("FORWARD-RIGHT ↖ (toward robot front)");
    break;
  case RL:
    Serial.println("BACKWARD-LEFT ↙ (toward robot rear)");
    break;
  case RR:
    Serial.println("BACKWARD-RIGHT ↘ (toward robot rear)");
    break;
  }

  Serial.print("  • When X is NEGATIVE (-): Leg pushes ");
  switch (legIndex) {
  case FL:
    Serial.println("BACKWARD-LEFT ↙ (pushes body forward)");
    break;
  case FR:
    Serial.println("BACKWARD-RIGHT ↘ (pushes body forward)");
    break;
  case RL:
    Serial.println("FORWARD-LEFT ↗ (pushes body forward)");
    break;
  case RR:
    Serial.println("FORWARD-RIGHT ↘ (pushes body forward)");
    break;
  }

  Serial.println("  • When Y decreases: Leg LIFTS (knee bends up) ↑");
  Serial.println("  • When Y increases: Leg LOWERS (foot contacts ground) ↓");
  Serial.println("\n🔍 OBSERVATION CHECK:");
  Serial.println("  1. Does the leg swing along its 45° mount axis?");
  Serial.println(
      "  2. When LED blinks ON, is the leg moving toward robot FRONT?");
  Serial.println("  3. If NO → Flip DIR_* constant for this leg");
}
...
private:
  ...
  void solveIK(float x, float y, float &kneeOut, float &hipOut) {
    ...
    float cosKnee = (distSq - UPPER_LEG_LEN * UPPER_LEG_LEN -
                     LOWER_LEG_LEN * LOWER_LEG_LEN) /
    ...
    float theta1 = atan2(y, x) - acos((UPPER_LEG_LEN * UPPER_LEG_LEN + distSq -
                                       LOWER_LEG_LEN * LOWER_LEG_LEN) /
    ...
    // These offsets align the mathematical model with your physical servo horn
    // mounting Adjust these values if the leg posture looks incorrect at
    // "neutral"
...
// Instantiate Legs
RobotLeg legFL(legsConfig[FL].pinKnee, legsConfig[FL].pinHip,
               legsConfig[FL].direction);
RobotLeg legFR(legsConfig[FR].pinKnee, legsConfig[FR].pinHip,
               legsConfig[FR].direction);
RobotLeg legRL(legsConfig[RL].pinKnee, legsConfig[RL].pinHip,
               legsConfig[RL].direction);
RobotLeg legRR(legsConfig[RR].pinKnee, legsConfig[RR].pinHip,
               legsConfig[RR].direction);
  ...
  if (currentState == STAND) {
    ...
    if (now - lastStepTime > GAIT_SPEED) {
      ...
      for (int i = 0; i < NUM_LEGS; i++) {
        ...
        // Y: Height - lift when at swing extremes (abs(cos) creates lift at
        // peaks)
